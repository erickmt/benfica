...
class Model_Venda {
    ...
    function buscarLojas($todas)
    {
          ...
          while ($linha = mysqli_fetch_array($resultado))
          {
              ...
              $item = array('id_loja' => $linha['id'], 'descricao' => $linha['descricao']); 
              $listaLoja[] = $item;
          }
    ...
    function gravarPedido($pedidoTiny, $id_cliente, $total, $resposta, $naoEmitir){
       if(isset($resposta["retorno"]['registros']['registro']['numero']))
        ...
        return array('indicador_erro' => 2, 'dados' => null);
    ...
    function listarPedidos($nome = false, $id_venda = false, $dataInicial = false, $dataFinal = false, $lojaBusca, $situacao, $idPagamento, $multiplasFormas){
        ...
        if($lojaBusca > 0){
          ...
          while ($linha   = mysqli_fetch_array($resultado))
          {                            
            $dados     = array('id_tiny' => $linha['id_tiny'], 'id_venda' => $linha['id_venda'], 'nome_cliente' => strtoupper($linha['nome_cliente']), 'total_venda' => $linha['total_venda'], 'situacao' => $linha['situacao'], 'retorno' => $linha['retorno'], 'emitida' => $linha['emitida'], 'nota' => $linha['nota'], 'data_pedido' => $linha['data_pedido'], 'link' => $linha['link']);              
            $retorno[] = $dados;
          }
      
          return array('indicador_erro' => 2, 'dados' => $retorno);
    }
          ...
          valor_credito_cliente,
				...
				values 
					(".$dados['id_cliente'].",".
					   ...
					   $dados['valor_total_comissao'].",".
					   $dados['valor_total_taxas'].",".
					   $dados['valor_total_outros'].",".
					   $dados['valor_total_liquido'].",".
             $dados['valor_credito_cliente'].",".
					   $dados['indicador_externo'].",".
					   $dados['indicador_consignado'].",".
					   $dados['taxa_pelo_cliente'].");";
  ...
  function buscarVendas($codigoVenda = false , $nomeCompleto = false , $cpf = false, $lojaLogada)
  {
          ...
          //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
                ...
                $dados              = array('id_venda' => $linha['id_venda'], 'dta_venda' => $linha['dta_venda'], 'nome' => strtoupper(($linha['nome'])), 'cpf' => $linha['cpf']);

                //Busca os produtos da venda em questão
                $sql_2 = 
                "select pro.descricao, ite.quantidade
    ...
    //Monta e executa a query
      ...
      //Encontrou uma ou mais vendas
       ...
       return array("retorno" => "sucesso", 'indicador_erro' => 0, 'dados' => $retorno);
     }
    ...
    function buscarUltimaVendaCliente($idVenda)
    {
          ...
          $resultado = $this->conexao->query($sql);
          if(!$resultado)
                return array('idCliente' => $linha['id_cliente'], 'data' => false);
          ...
          return array('idCliente' => $linha['id_cliente'], 'data' => $linha2['dta_venda']);
  }
  ...
  function buscarVendasComissao($idVendedor, $mes, $ano, $lojaBusca, $idPagamento)
  {
          ...
          //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
                ...
                $dados     = array('lj_venda' => $linha['lj_venda'], 'id_venda' => $linha['id_venda'], 'dta_venda' => $linha['dta_venda'], 'nome' => strtoupper(($linha['nome'])), 'numero_rg' => $linha['numero_rg'], 'valor_total_pago' => $linha['valor_total_venda'], 'valor_total_comissao' => $linha['valor_total_comissao'], 'per_desc' => $linha['per_desc'] );
        ...
        if($idVendedor == 0 && $lojaBusca == 0){
          ...
          //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
                ...
                $dados     = array('lj_venda' => $linha['lj_venda'], 'nome' => strtoupper(($linha['nome'])), 'total_venda' => $linha['total_venda'], 'comissao_total' => $linha['comissao_total'], 'total_venda_varejo' => $linha['total_venda_varejo'], 'comissao_varejo' => $linha['comissao_varejo'], 'total_venda_atacado' => $linha['total_venda_atacado'], 'comissao_atacado' => $linha['comissao_atacado'] );
  ...
  function buscarVendasPorFormaPagamento($dataInicial, $dataFinal, $lojaBusca)
  {
          ...
          //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
                ...
                $dados     = array('descricao' => $linha['descricao'], 'valor' => number_format($linha['valor'], 2, ',', '.'));
                $retorno[] = $dados;
                $valor     = $valor + $linha['valor'];
           }

           $valor         = number_format($valor, 2, ',', '.');
           $retorno[]     = array('descricao' => 'TOTAL', 'valor' => $valor);
     ...
     function buscarPecas($tipoProduto, $dataInicial, $dataFinal, $produto, $lojaBusca ,$tipoRelatorio)
    {
      ...
      //Encontrou uma ou mais vendas
        ...
        while ($linha   = mysqli_fetch_array($resultado))
        {
            ...
            $dados     = array('descricao' => $linha['descricao'], 'valor' => $linha['valor'], 'estoque' => $linha['estoque'], 'desc_tipo' => $linha['desc_tipo'], 'data' => $linha['data'] );
            $retorno[] = $dados;
            $valor     = $valor + $linha['valor'];
        }

        $retorno[]     = array('descricao' => 'TOTAL', 'desc_tipo' => '', 'valor' => $valor, 'estoque' => '');
          ...
          //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
                if($linha['data'] != $dt_ant){
                  if($total_data != 0){
                    $dados     = array('data' => '', 'descricao' => 'TOTAL', 'valor' => round($total_data,2));
                    $retorno[] = $dados;
                    $total_data = 0;
                  }
                  $dados     = array('data' => $linha['data'], 'descricao' => 'Vendas do dia', 'valor' => $linha['v_venda']);
                ...
                $dados     = array('data' => $linha['data'], 'descricao' => $linha['descricao'], 'valor' => round($linha['valor'],2));
                $retorno[] = $dados;
                $valor     = $valor + $linha['valor'];
                $ult_dt = $linha['data'];
           }
           $dados     = array('data' => '', 'descricao' => 'TOTAL', 'valor' => round($total_data,2));
           $retorno[] = $dados;
           $retorno[]     = array('data' => 'TOTAL', 'descricao' => '', 'desc_tipo' => '', 'valor' => round($valor,2));
  ...
  function buscarFaturamento($dataInicial, $dataFinal, $lojaBusca)
  {
		  ...
		  //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
              ...
              $dados     = array('mes' => $linha['mes'], 'valor_comissao' => $linha['valor_comissao'], 'valor_taxas' => $linha['valor_taxas'], 'valor_outros' => $linha['valor_outros'], 'valor_liquido' => $linha['valor_liquido'], 'valor_pago' => $linha['valor_pago']);
                $retorno[] = $dados;
           }
  ...
  function buscarFaturamentoAno($ano, $lojaBusca)
  {
      ...
      //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
              ...
              $dados     = array('trimestre' => $linha['trimestre'], 'valor_comissao' => $linha['valor_comissao'], 'valor_taxas' => $linha['valor_taxas'], 'valor_outros' => $linha['valor_outros'], 'valor_liquido' => $linha['valor_liquido'], 'valor_pago' => $linha['valor_pago']);
                $retorno[] = $dados;
           }
  ...
  function buscarCusto($dataInicial, $dataFinal, $lojaBusca)
  {
      ...
      //Encontrou uma ou mais vendas
           ...
           while ($linha   = mysqli_fetch_array($resultado))
           {
              ...
              $dados     = array('valor_custo' => $linha['valor_custo']);
                $retorno[] = $dados;
           }
  ...
  function buscarValoresGeraisVendas($dataInicial, $dataFinal, $lojaBusca)
  {
          ...
          $retorno = array('valor_comissao' => $linha['valor_comissao'], 'valor_taxas' => $linha['valor_taxas'], 'valor_outros' => $linha['valor_outros'], 'valor_liquido' => $linha['valor_liquido'], 'valor_custo' => $linha2['valor_custo']);
          return array('indicador_erro' => 0, 'dados' => $retorno);
  }
